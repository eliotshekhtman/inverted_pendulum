...
def main() -> None:
    ...
    all_episode_residuals_baseline: list[np.ndarray] = []
    all_episode_theta_baseline: list[np.ndarray] = []
    all_episode_thetad_baseline: list[np.ndarray] = []
    all_episode_v_baseline: list[np.ndarray] = []

    for episode in range(num_episodes):
        used_r_history.append(r_j)
        episode_seed = int(rng.integers(0, 2**32 - 1))
        rng_robust = np.random.default_rng(episode_seed)
        rng_baseline = np.random.default_rng(episode_seed)
        ...
        (
            ...
            use_random_init=True,
            show_progress=True,
            progress_desc=f"Episode {episode:02d} baseline",
        )
        ...
        all_episode_residuals_baseline.append(np.asarray(traj_residuals_baseline, dtype=np.float64))
        all_episode_theta_baseline.append(np.asarray(traj_theta_baseline, dtype=np.float64))
        all_episode_thetad_baseline.append(np.asarray(traj_thetad_baseline, dtype=np.float64))
        all_episode_v_baseline.append(np.asarray(traj_v_baseline, dtype=np.float64))
    ...
    np.savez(
        ...
        theta_baseline=np.asarray(all_episode_theta_baseline, dtype=np.float64),
        thetad_baseline=np.asarray(all_episode_thetad_baseline, dtype=np.float64),
        v_baseline=np.asarray(all_episode_v_baseline, dtype=np.float64),
        residuals_baseline=np.asarray(all_episode_residuals_baseline, dtype=np.float64),
        dt=float(dt),
        steps=int(steps),
