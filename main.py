...
def main() -> None:
    ...
    p_sym_error = float(np.linalg.norm(controller.P - controller.P.T, ord="fro"))
    ...
    for episode in range(num_episodes):
        ...
        # 1) Calibration rollouts for quantile/margin update only (not saved for plotting).
        traj_residuals_calib, _, _, _ = collect_trajectories(
            env=env,
            controller=controller,
            r_j=r_j,
            num_trajs=num_calibration_trajs,
            steps=steps,
            dt=dt,
            rng=rng_calib,
            use_random_init=True,
            show_progress=True,
            progress_desc=f"Episode {episode:02d} calibration",
        )
        ...
        traj_residuals, traj_theta, traj_thetad, traj_v = collect_trajectories(
            ...
            use_random_init=True,
            show_progress=True,
            progress_desc=f"Episode {episode:02d} evaluation",
        )
    ...
    np.savez(
        ...
        num_trajs_per_episode=int(num_eval_trajs),
        num_calibration_trajs=int(num_calibration_trajs),
        num_eval_trajs=int(num_eval_trajs),
        seed=int(seed),
        baseline_seed=int(baseline_seed),
        k_fb=np.asarray(controller.k_fb, dtype=np.float64),
        P=np.asarray(controller.P, dtype=np.float64),
        p_eigvals=np.asarray(p_eigs, dtype=np.float64),
        p_sym_error=p_sym_error,
